...
class _CreateGroupScreenState extends State<CreateGroupScreen> {
  ...
  void initState() {
    ...
    print('Friends data: ${widget.friends.map((f) => 'friendId: ${f.friendId}, username: ${f.username}').toList()}');
    ...
    for (var friend in _filteredFriends) {
      if (friend.friendId == null) {
        print('Warning: Friend with username ${friend.username} has null friendId');
      }
    }
    _filteredFriends.sort((a, b) => (a.username ?? '').compareTo(b.username ?? ''));
  ...
  void _filterFriends() {
    ...
    setState(() {
      ...
      print('Filtered friends count: ${_filteredFriends.length}');
      print('Filtered friends: ${_filteredFriends.map((f) => 'friendId: ${f.friendId}, username: ${f.username}').toList()}');
    });
  }
  ...
  void _createGroup() async {
    if (_groupNameController.text.isEmpty || _selectedFriendIds.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Vui lòng nhập tên nhóm và chọn ít nhất một thành viên'),
                            ...
                            return TweenAnimationBuilder(
                              ...
                              child: Card(
                                ...
                                child: CheckboxListTile(
                                  ...
                                  value: friend.friendId != null &&
                                      _selectedFriendIds.contains(friend.friendId),
                                  onChanged: (selected) {
                                    if (friend.friendId == null) {
                                      ScaffoldMessenger.of(context).showSnackBar(
                                        const SnackBar(
                                          content: Text('Lỗi: Bạn bè không có friendId hợp lệ'),
                                          backgroundColor: Colors.redAccent,
                                          duration: Duration(seconds: 2),
                                        ),
                                      );
                                    ...
                                    setState(() {
                                      if (selected == true &&
                                        ...
                                        print('Added friendId: ${friend.friendId}, Selected: $_selectedFriendIds');
                                      } else if (selected == false) {
                                        _selectedFriendIds.remove(friend.friendId);
                                        print('Removed friendId: ${friend.friendId}, Selected: $_selectedFriendIds');
                                      }
                                    });
                                  },
