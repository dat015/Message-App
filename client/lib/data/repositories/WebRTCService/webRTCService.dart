...
class WebRTCService {
  ...
  WebRTCService(this._webSocketService, this._userId, this._conversationId, {this.onRemoteStreamAdded});
  ...
  Future<void> init() async {
    try {
      ...
      print('Renderers initialized: local=${_localRenderer.srcObject}, remote=${_remoteRenderer.srcObject}');
      ...
      print('Local stream obtained: id=${_localStream?.id}, tracks=${_localStream?.getTracks().map((t) => "${t.kind}: enabled=${t.enabled}").join(", ")}');

      _localStream?.getTracks().forEach((track) {
        _peerConnection?.addTrack(track, _localStream!);
        print('Track added: id=${track.id}, kind=${track.kind}, enabled=${track.enabled}');
      });
      ...
      _peerConnection?.onTrack = (RTCTrackEvent event) {
        print('onTrack triggered: stream id=${event.streams[0].id}, tracks=${event.streams[0].getTracks().map((t) => "${t.kind}: enabled=${t.enabled}").join(", ")}');
      ...
      _peerConnection?.onAddStream = (stream) {
        print('onAddStream triggered: stream id=${stream.id}, tracks=${stream.getTracks().map((t) => "${t.kind}: enabled=${t.enabled}").join(", ")}');
    ...
    try {
      ...
      _peerConnection!.getRemoteDescription().then((remoteDesc) {
        if (remoteDesc != null) {
          ...
          print('Remote description not set, queuing ICE candidate: ${data['candidate']}');
